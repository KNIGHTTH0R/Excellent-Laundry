	...
	<div class="login-box">
	    <h2>Login</h2>
            <div class="form-group <?php if($kUser->arErrorMessages['usEmail'] !=''){ ?> has-error <?php } ?>">
                    ...
                    <div class="input-box">
                        <input class="form-control placeholder-no-fix" autocomplete="off" id="usEmail" name="loginAry[usEmail]"  type="text" placeholder="Email ID" onfocus="remove_formError('usEmail')" value="<?php if(!empty($_POST['loginAry']['usEmail'])){ echo $_POST['loginAry']['usEmail']; } ?>" onkeypress="ajaxKeyUpLogin(this,event);" /> 
                    </div>
